...
data <-read.fasta(file = "TIE1_sequence.fasta copy", seqtype = "DNA", as.string = TRUE, legacy.mode = TRUE, seqonly = TRUE, strip.desc = TRUE, whole.header = TRUE)
```
...
#The following steps contains help from internet and Chat bot, I was just very confused
...
Now I would like to translate the sequence into protein:
```{r}
aa_ref <- as.character(translate(DNAString(codon_ref)))
aa_mut <- as.character(translate(DNAString(codon_mut)))
```
